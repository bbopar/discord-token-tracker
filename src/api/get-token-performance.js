 ...
 */
async function getTokenPerformance(tokenAddress, apiKey, options = {}) {
  ...
  for (let attempt = 1; attempt <= PROVIDER_CONFIG.MAX_RETRIES; attempt++) {
    try {
      ...
      console.log("TRADE DATA:", tradeDataResponse);
 ...
 */
async function batchGetTokenPerformance(tokenAddresses, apiKey, options = {}) {
  const results = {};
 ...
 * @returns {Promise<void>}
 */
async function updateTokenPerformance(dataHandler, tokenAddress, apiKey) {
  try {
    ...
    const tokenData = await getTokenLiquidity(tokenAddress, process.env.BIRDEYE_API_KEY);
    ...
    const tokenPerformance = {
        ...
        priceChange24h: metrics.price_change_24h_percent || 
        ...
        volumeChange24h: metrics.volume_24h_change_percent ||
        ...
        trade_24h_change: metrics.trade_24h_change_percent ||
        ...
        holderChange24h: metrics.unique_wallet_24h_change_percent ||
        ...
        marketCapChange24h: ((metrics.price * performanceData.security.data.totalSupply) - 
                            (metrics.history_24h_price * performanceData.security.data.totalSupply)) / 
                            (metrics.history_24h_price * performanceData.security.data.totalSupply) * 100 || 0,
    
        // Sustained growth - check multiple timeframes but be more lenient if some are missing
        sustainedGrowth: (metrics.price_change_24h_percent > 0 || !metrics.price_change_24h_percent) &&
                         (metrics.price_change_12h_percent > 0 || !metrics.price_change_12h_percent) &&
                         (metrics.price_change_6h_percent > 0 || !metrics.price_change_6h_percent) &&
                         (metrics.price_change_4h_percent > 0),
    
        // Rapid dump - check shorter timeframes if longer ones aren't available
        rapidDump: (metrics.price_change_1h_percent < -20) ||
                   (metrics.price_change_30m_percent < -15) ||
                   (metrics.price_change_24h_percent < -50),
    
        // Suspicious volume - consider multiple timeframes
        suspiciousVolume: (metrics.volume_24h_usd > 1000000) ||
                         (metrics.volume_6h_usd > 500000) ||
                         (metrics.volume_1h_usd > 250000) ||
                         Math.abs(metrics.volume_24h_change_percent || 
                                 metrics.volume_1h_change_percent || 0) > 500,
    
        validationTrust: 0,
        balance: performanceData.security.data.creatorBalance || 0,
        initialMarketCap: performanceData.security.data.totalSupply * metrics.price || 0,
    };

    // Update cache using DataHandler
    await dataHandler.updateTokenPerformance(tokenAddress, tokenPerformance);
 ...
 */
async function getTokenLiquidity(tokenAddress, apiKey) {
  const client = createApiClient(apiKey);
  let lastError = null;
  ...
  for (let attempt = 1; attempt <= PROVIDER_CONFIG.MAX_RETRIES; attempt++) {
    try {
      ...
      const response = await client.get(`/defi/token_overview?address=${tokenAddress}`);
...
module.exports = {
  updateTokenPerformance,
  getTokenPerformance,
  batchGetTokenPerformance,
};
