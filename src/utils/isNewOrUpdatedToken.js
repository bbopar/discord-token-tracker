export async function isNewOrUpdatedToken(message, dataHandler) {
    const existingToken = await dataHandler.getTokenByAddress(
    ...
    if (message.token.stats) {
        const lastUpdate =
            existingToken.updates[existingToken.updates.length - 1];
        return (
            !lastUpdate ||
